...
class MicroserviceOption:
    ...
    option: Optional[
        Union[
            RedisClientOption, MqttClientOption, NatsClientOption,
    ...
    def __post_init__(self):
        if self.transport == Transport.TCP and self.option is not None and not isinstance(
                self.option, TCPClientOption):
            raise ValueError("Option must be of type TCPClientOption for TCP transport")
        elif self.transport == Transport.REDIS and self.option is not None and not isinstance(
                self.option,
                RedisClientOption
        ):
            raise ValueError("Option must be of type RedisClientOption for REDIS transport")
        elif self.transport == Transport.MQTT and self.option is not None and not isinstance(
                self.option,
                MqttClientOption
        ):
            raise ValueError("Option must be of type MqttClientOption for MQTT transport")
        elif self.transport == Transport.NATS and self.option is not None and not isinstance(
                self.option,
                NatsClientOption
        ):
            raise ValueError("Option must be of type NatsClientOptions for NATS transport")
        elif self.transport == Transport.RABBITMQ and self.option is not None and not isinstance(
                self.option,
                RabbitMQClientOption
        ):
            raise ValueError("Option must be of type RabbitMQClientOption for RABBITMQ transport")

        elif self.transport == Transport.GRPC and self.option is not None and not isinstance(
                self.option,
                GrpcClientOption
        ):
            raise ValueError("Option must be of type GrpcClientOption for GRPC transport")
...
class ClientProxy(ABC):
    ...
    async def emit(self, topic, data, headers: dict[str, str] = None):
        ...
        await self._publish(f"{MICROSERVICE_CHANNEL}:{self.option.channel_key}", json_req)
