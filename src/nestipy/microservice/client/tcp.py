    ...
    async def handle_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        try:
            while True:
                ...
                command, topic, *message = data.decode('utf-8').strip().split(__SPLIT__, 2)
...
class TCPClientProxy(ClientProxy):
    ...
    def __init__(self, option: MicroserviceOption):
        ...
        self._config = typing.cast(TCPClientOption, self.option.option or TCPClientOption())
        self.server: Optional[TCPServer] = TCPServer(
            host=self._config.host,
            port=self._config.port,
            verbose=self._config.verbose
        ) if self._config.start_server else None
    ...
    async def connect(self):
        ...
        if not self.writer:
            self.reader, self.writer = await asyncio.open_connection(self._config.host, self._config.port)
    ...
    async def _publish(self, topic: str, data: str):
        ...
        await self.connect()
        self.writer.write(f'PUBLISH{__SPLIT__}{topic}{__SPLIT__}{data}\n'.encode('utf-8'))
